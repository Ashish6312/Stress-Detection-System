...
def ml_predict_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            features_dict = {
                'BVP_mean': data.BVP_mean,
                'BVP_std': data.BVP_std,
                'BVP_peak_freq': data.BVP_peak_freq,
                'EDA_phasic_mean': data.EDA_phasic_mean,
                'EDA_phasic_min': data.EDA_phasic_min,
                'EDA_smna_min': data.EDA_smna_min,
                'EDA_tonic_mean': data.EDA_tonic_mean,
...
def resources_view(request):
    resources = [
        {'title':'5-minute breathing exercise','url':'https://www.youtube.com/watch?v=eo7w5aM1u6c'},
